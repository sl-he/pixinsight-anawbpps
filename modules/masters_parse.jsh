...
function MP_parseMaster(path){
    ...
    // ---- gain / offset / usb ----
    var gain = null, offset = null, usb = null;
    for (var g = 0; g < tokens.length; ++g){
        ...
        var mU  = tg.match(/^U(\d+)$/i);      if (mU)  usb = parseInt(mU[1],10);
//        var mUSB= tg.match(/^USB(\d+)$/i);    if (mUSB) usb = parseInt(mUSB[1],10);
    }
